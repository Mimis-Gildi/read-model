...
import me.riddle.adventure.web.model.status.ControlEvent
...
object PerformanceScoreTable {
    ...
    suspend fun record(message: Incoming) = when (message) {
        is Report -> recordReport(message)
        is ControlEvent -> recordControlEvent(message)
    }

    private suspend fun recordReport(report: Report) {
    ...
    private fun recordControlEvent(controlEvent: ControlEvent) {
