...
class ReportTest {
    ...
    companion object {
        ...
        val reportOnTheWire by lazy {
            """
                {
                  "$PARAMETER_TYPE": "$DATA_KEY_REPORT",
                  "$PARAMETER_RUN_ID": "1787955391293-5501f6aa1128b8",
                  "$PARAMETER_UI_FRAMEWORK": "${UI_FRAMEWORK_PURE_TS.first}",
                  "$PARAMETER_DATASET": "$DATASET_KEY_SMOKE",
                  "rung": "Teams",
                  "elements": 2200,
                  "built": 12.5,
                  "painted": 3.25
    ...
    fun `should serialize with defaulted type`() = assertEquals(
        reportOnTheWire,
        prettyJson.encodeToString<Incoming>(report).also { tLogger.trace { "Report is:\n\n$it" } },
    ...
    fun `should deserialize hand-json to report`() = assertEquals(
        report,
        Json.decodeFromString<Incoming>(reportOnTheWire),
