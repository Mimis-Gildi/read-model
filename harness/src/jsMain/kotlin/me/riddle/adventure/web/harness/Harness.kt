...
external interface Fixture {
    ...
    /** Unfolds the next chunk of up to [count] folded `Team`s of `Person` nodes. Returns how many it actually unfolded. */
...
/** One BUILD measured render (a ladder rung: build ON clock in `Expand`, then painted). */
data class Measurement(val elements: Int, val built: Double, val painted: Double)

/** One REVEAL measured chunk of the reveal: no build toggling on already-built DOM classes for `Pure`; and opposite for `React` and `Kobweb`. */
...
/**
 * The "rung" in the result row. These four "level" names materialize in the fixture's HTML exactly once, and in the
...
/** The ladder's rungs are read off of DOM (state in React) in the depth order. */
 ...
 */
private val socket = WebSocket(EVENT_PROTOCOL + window.location.host + EVENT_CHANNEL)
